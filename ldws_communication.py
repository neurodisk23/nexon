...
parameters = np.load('parameters.npz') # Has the values ret, mtx, dist, rvecs, tvecs
global mtx, dist
mtx  = parameters['mtx']
...
def lane_fill_poly(binary_warped,undist,left_fit,right_fit):
    ...
    # Draw the lane 
    cv.fillPoly(color_warp, np.int_([pts]), (250,0, 0))
...
while True:
    ...
    draw_poly = cv.polylines(img,[source_points],True,(0,0,255), 3)

    cv.imshow('Working Space',draw_poly)
    cv.moveWindow('Working Space',0,0)

    cv.imshow('Brids-Eye View',birdseye_result*250)
    cv.moveWindow('Brids-Eye View',1000,0)
    ...
    curvature =  measure_curve(birdseye_result,left_fit, right_fit)
    print('/n The road curvature is: ', curvature)

    offset = vehicle_offset(colored_lane, left_fit, right_fit)

    if offset < -0.3 :
        message = "A2,N,0,0,0,0,0,1,0,0,0\r\n"
        send_data(ip_address, port, message,2)
    if offset < -0.4 :
        message = "A2,N,0,0,0,0,0,1,0,0,0\r\n"
        send_data(ip_address, port, message,2)

    print("/n The vehcicle offset is: ", offset)

    
    offset_text = 'Offset: ' + str(offset) + ' Curvature: ' + str(curvature)
    cv.putText(colored_lane,offset_text, 
    ...
    cv.imshow('Detected Line',colored_lane)
    cv.resizeWindow('Detected Line', 1000,1000)
    cv.moveWindow('Detected Line ',0,1000)
